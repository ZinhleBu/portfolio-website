...
export const ImageUpload = ({
  ...
  const onUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const uploadTask = uploadBytesResumable(
      ref(storage, `ProjectCoverImage/${Date.now()}-${file?.name}`),
      file,
        ...
        <>
          <div className="w-full h-60 aspect-video relative rounded-md flex items-center justify-center overflow-hidden">
            <Image
              ...
              alt="Image cover"
            />
            <div className="absolute z-20 top-2 right-2 cursor-pointer" onClick={onDelete}>
        ...
        <>
          <div className="w-full h-60 aspect-video relative rounded-md flex items-center justify-center overflow-hidden border border-dashed bg-neutral-50">
            {isLoading ? (
              ...
              <>
                <label>
                  <div className="w-full h-full flex flex-col gap-2 items-center justify-center cursor-pointer text-neutral-400">
                    <ImagePlus strokeWidth={1} className="w-10 h-10 " />
                    <p>Upload an image</p>
                  </div>
                  <input
                    type="file"
