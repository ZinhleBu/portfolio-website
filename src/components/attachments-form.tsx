...
import { Project, Attachment } from "@prisma/client";
...
interface AttachmentsFormProps {
  initialData: Project & { attachments: Attachment[] };
  projectId: string;
}
...
export const AttachmentsForm = ({
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      toast.success("Project Attachment updated");
      toggleEditing();
      router.refresh();
    } catch (error) {
  ...
  const onDelete = async (attachment: Attachment) => {
    try {
      ...
      await axios.delete(`/api/projects/${projectId}/attachments/${attachment.id}`);
      toast.success("Project Attachment Removed");
      router.refresh();
    } catch (error) {
      console.log((error as Error)?.message);
      toast.error("Something went wrong");
